          ...
          {filteredUsers.map((user) => (
            ...
            >
              <div className={styles["avatar-container"]}>
                ...
                {onlineUsers.has(user.id) && <div className={`${styles["status-indicator"]} ${styles.online}`} />}
              </div>
              <div className={styles["user-details"]}>
                <span className={styles["user-name"]}>{user.name}</span>
                <span className={styles["last-msg"]}>Clique para conversar</span>
              </div>
            </div>
          ))}
          {filteredUsers.length === 0 && (
            <p className={styles["empty-list"]}>Nenhum usuário encontrado.</p>
          )}
        </div>
      )}
    </aside>
